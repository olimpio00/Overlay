...
var yt_warning_show_count = get_cookie('yt_warning_show_count') || 0;
class iframe_element extends element {
  constructor(parent, data) {
    ...
    this.iframe = $(
      `<iframe class="iros-iframe" src="${this.data.url}" frameborder="0"></iframe>`
    );
  ...
  set_url(url) {
    ...
    if (url.indexOf('youtube.com') !== -1 || url.indexOf('youtu.be') !== -1) {
      // notify the user that youtube videos are not synced
      if (yt_warning_show_count < 2) {
        alert(
          `YouTube videos are not synced across clients. The video will only play for you. If you need synchronized video playback consider uploading the video somewhere as a file and embed it using the video element with a direct link to the video file (i.e. ending in .mp4/.webm etc.). This warning will be shown only twice.`
        );
        yt_warning_show_count++;
        set_cookie('yt_warning_show_count', yt_warning_show_count, 365);
      }
    }
...
class iframe_element_handler extends element_handler {
  ...
  update_selected_element() {
    if (this.selected_element) {
      ...
      changed |= this.selected_element.set_block_events(
        this.block_events.checked
      );
      if (changed) send_command_update_element(this.edt, this.selected_element);
    }
  }
