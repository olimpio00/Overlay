...
class text_element extends element {
  constructor(parent, data, type = 'text') {
    ...
    this.html = $(
      `<div class="iros-element iros-text-element" id="${
        this.data.id
      }">${this.get_formatted_text()}</div>`
    );
  }
  ...
  update() {
    ...
    this.html.style.transformOrigin = `${this.tf().h_pivot} ${
      this.tf().v_pivot
    }`;
  }
...
class text_element_handler extends element_handler {
  constructor(edt, type = 'text') {
    ...
    this.enable_background_color.on('input', () =>
  ...
  update_selected_element() {
    if (this.selected_element) {
      ...
      this.selected_element.data.background_color =
        this.background_color.value +
        (this.enable_background_color.checked ? 'ff' : '00');
  ...
  show_settings(element) {
    ...
    this.enable_background_color.checked =
      element.data.background_color.substring(7) === 'ff';
